...
def process_data():
    ...
    with open('../2024_04_01_DATA_withGroupinfo_with_treatment_and_description_sort_final_xifen.csv') as f:
        ...
        for row in reader:
            ...
            age_xifen = normalize_field(row['Age_xifen'])
            tissue = normalize_field(row['Tissue.1'])
            
            # 构建分组名称
            group = f"{age}__{age_xifen}__{tissue}"
            run_group_mapping[row['Run']] = group

    with open('Bombyx_mori_rpkm_values_origin.txt') as fin, \
         open('GeneExpression_GroupedData.tsv', 'w') as fout:
...
if __name__ == "__main__":
    process_data()
    print("处理完成，结果已保存到 GeneExpression_GroupedData.tsv")
