...
# 转换宽表为长表，拆解列名中的结构
long_data <- target_genes[["Acyrthosiphon_pisum"]] %>%
  pivot_longer(
...
long_data <- long_data %>%
  mutate(
    Age = if_else(Age == "Larva", "Nymph", Age)
  )
...
# 自定义顺序（注意：重复项将自动去除）
custom_order <- c(
  "Egg-17_hours", "Egg-18_hours","Egg-stage_17","Egg-stage_18","Egg-stage_19","Egg-stage_20","Egg-stage_21","Egg-stage_22","Egg-stage_23","Egg-stage_24",
  "Nymph-1_instar","Nymph-early_2_instar","Nymph-2_instar","Nymph-3_instar","Nymph-4_instar",
  "Adult-2_days","Adult-3_days","Adult-4_days","Adult-6_days","Adult-7_days","Adult-2-6_days","Adult-2-7_days","Adult-2-8_days","Adult-9_days","Adult-11_days"
)
...
ggsave(
  filename = "Acyrthosiphon_pisum_LOC100169075.pdf",  # 文件路径和名称
