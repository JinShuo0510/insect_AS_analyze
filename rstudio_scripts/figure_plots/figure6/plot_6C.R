...
# 1. 计算每个物种的总 exon 数
species_totals <- summary_stats_psi %>%
  group_by(species) %>%
  summarise(species_total = sum(count, na.rm = TRUE),
            .groups = "drop")

# 2. 提取目标数据并计算比例
plot_data <- summary_stats_psi %>%
  filter(
...
# 3. 计算 Wilson 95% 置信区间（以 species_total 作为分母）
plot_data <- plot_data %>%
  rowwise() %>%
  mutate(
    ci = list(binom.confint(count, species_total, conf.level = 0.95, methods = "wilson")),
    lower = ci$lower,
    upper = ci$upper
  ) %>%
  ungroup() %>%
...
for(tissue in c("Head", "Whole_body")) {
  ...
  tot <- sum(d$species_total)  # 总分母等于所有选中物种的 species_total 之和
  test <- prop.test(c(total_h, total_s), c(tot, tot))
  p <- test$p.value
  p_fmt <- if(p < 0.001) {
...
for (tissue in c("Head", "Whole_body")) {
  ...
  print(p)
  ggsave(
    filename = paste0("psi_exon_type_", tissue, ".pdf"),
    plot = p,
