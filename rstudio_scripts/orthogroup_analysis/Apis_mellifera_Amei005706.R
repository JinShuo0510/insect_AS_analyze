...
# 转换宽表为长表，拆解列名中的结构
long_data <- target_genes[["Apis_mellifera"]] %>%
  pivot_longer(
...
# 自定义顺序（注意：重复项将自动去除）
custom_order <- c(
  "Egg-2_hours", "Egg-24_hours","Egg-32_hours",
  "Larva-2_days","Larva-6_days","Larva-5_instar",
  "Pupa-2_days",
  "Adult-1_days", "Adult-5_days","Adult-7_days", "Adult-10_days","Adult-1-7_weeks","Adult-advanced_overwinter_stage","Adult-initial_overwinter_stage"
)
...
final_data <- long_data_cleaned %>%
  ...
  # 2. 筛选指定区域（根据需要调整）
  filter(exonStart_0base == 5036970   ) %>%
  # 3. 拆分 PSI 列
...
ggsave(
  filename = "Apis_mellifera_Amei005706.pdf",  # 文件路径和名称
