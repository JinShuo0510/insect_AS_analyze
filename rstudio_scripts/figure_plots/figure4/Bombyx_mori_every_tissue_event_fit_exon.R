...
run_event_analysis <- function(tissue_name) {
  ...
  # 构造事件标识符（便于后续对比）
  df_tissue <- df_tissue %>%
    mutate(event_id = paste(GeneID, chr, strand, exonStart_0base, exonEnd, sep = "__"))
...
# 检查 results 是否存在，如果不存在则初始化
if (!exists("results")) {
  results <- list()
}

# 遍历每个 Tissue，运行分析并将结果追加到列表中
results <- append(results, map(tissues, function(tissue) {
  res <- run_event_analysis(tissue)
  pb$tick()  # 更新进度条
  return(res)
}))
names(results) <- c(names(results), tissues)

if (tail(names(results), 1) == "") {
  names(results)[length(names(results))] <- "Whole_body"
}


# 创建一个新的列表来存储处理后的结果
filtered_results <- lapply(results, function(tissue_df) {
  # 确定要移除的列名
  cols_to_remove <- c("data", "model", "newdata", "PSI_pred", "delta_PSI","time_vec", "null_model")
  
  # 确保这些列确实存在于数据中
  cols_to_remove <- intersect(cols_to_remove, colnames(tissue_df))
  
  # 移除指定列
  tissue_df[, !colnames(tissue_df) %in% cols_to_remove, drop = FALSE]
})

# 如果需要保留原始列表结构，可以这样命名
names(filtered_results) <- names(results)

saveRDS(filtered_results, "Bombyx_mori_event_fits_all_exon.RDS")
