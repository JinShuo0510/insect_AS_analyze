#!/usr/bin/env Rscript
# ─────────────────────────────────────────────────────────────────────────────
# 并行 + 向量化 DTU 流程  - 高阈值版（含 Age、Tissue、Pair 三类结果 & MeanCPM_A/B）
...
# 1. 定义输入目录
age_dir    <- "/data/home/jinshuo/jupyter-notebook/DEG_and_DET/edgeR_DTU_voom_highThresh_groupmean/Age/Bombyx_mori"
tissue_dir <- "/data/home/jinshuo/jupyter-notebook/DEG_and_DET/edgeR_DTU_voom_highThresh_groupmean/Tissue/Bombyx_mori"
pair_dir   <- "/data/home/jinshuo/jupyter-notebook/DEG_and_DET/edgeR_DTU_voom_highThresh_groupmean/pair/Bombyx_mori"
...
# 3. 定义函数：读取单个文件并格式化，新增 MeanCPM_A, MeanCPM_B
...
fwrite(result_dt,
       file = "DTx_nonDEG_orthogroups_matched_dt_updated_groupmean_1.tsv",
       sep  = "\t")
