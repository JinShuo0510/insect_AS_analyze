...
# 转换宽表为长表，拆解列名中的结构
long_data <- target_genes[["Aedes_aegypti"]] %>%
  pivot_longer(
...
# 自定义顺序（注意：重复项将自动去除）
custom_order <- c(
  "Egg-0-1_hours", "Egg-2-4_hours",
  "Adult-3_days","Adult-3-5_days","Adult-4-7_days","Adult-7_days","Adult-8_days"
)
...
final_data <- long_data_cleaned %>%
  ...
  # 2. 筛选指定区域（根据需要调整）
  # filter(exonStart_0base == 322375847 | exonStart_0base == 322383912 | exonStart_0base ==  322381456) %>%
  # 3. 拆分 PSI 列
...
ggsave(
  filename = "Aedes_aegypti_g18888.pdf",  # 文件路径和名称
