...
output_root       <- "edgeR_DTU_voom_highThresh"
libsize_thresh    <- 1e6
workers           <- 4
...
process_species <- function(sp) {
    ...
    for (fac in c("Age","Tissue")) {
        g_res <- if (fac == "Age") gene_age else gene_tissue
        t_res <- if (fac == "Age") tx_age   else tx_tissue
