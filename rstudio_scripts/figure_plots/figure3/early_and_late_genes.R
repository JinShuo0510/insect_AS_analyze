...
# 1. 读入数据
data <- read.table("GeneExpression_GroupedData_origin.tsv",
...
for (tissue in unique_tissues) {
  ...
  # 绘制聚类中心图
  output_file <- paste0("mfuzz_clusters_", tissue, ".png")
  ...
  # 绘制带趋势标记的聚类图
  output_file_trend <- paste0("mfuzz_clusters_trend_", tissue, ".png")
  ...
  if (length(early_genes_tissue) > 0) {
    ...
    write.csv(all_early_tissue, paste0(tissue, "_early_genes.csv"), row.names = FALSE)
    message(paste("已保存组织", tissue, "的早期基因至", paste0(tissue, "_early_genes.csv")))
  }
  
  if (length(late_genes_tissue) > 0) {
    all_late_tissue <- do.call(rbind, late_genes_tissue)
    late_genes_by_tissue[[tissue]] <- all_late_tissue
    write.csv(all_late_tissue, paste0(tissue, "_late_genes.csv"), row.names = FALSE)
    message(paste("已保存组织", tissue, "的晚期基因至", paste0(tissue, "_late_genes.csv")))
  }
}

# 保存趋势摘要
write.csv(summary_data, "gene_cluster_trends_summary.csv", row.names = FALSE)
...
# 保存合并的早期/晚期基因列表
process_and_save_gene_list(all_early_genes, "early_genes_all_tissues.csv")
process_and_save_gene_list(all_late_genes, "late_genes_all_tissues.csv")

message("分析完成！所有结果已保存。")